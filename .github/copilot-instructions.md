...
- **No build/test/lint tooling exists.** Changes are made directly to HTML/CSS and pushed. To preview locally, open the HTML files directly in a browser.
...
- **Page `<head>` boilerplate** is repeated in every file: charset, viewport, title, OG/Twitter meta tags, favicons, stylesheet link, and Google Fonts preconnect. When adding a new page, copy the full `<head>` from an existing page and update the title, description, OG URL, and OG description.
...
- **Publication entries** use a consistent structure: `.pub-item` containing `.pub-title`, `.pub-authors`, `.pub-venue`, and optional `.pub-note`, `.pub-award`, `.pub-status`. Highlighted publications get the `.highlight` class. Expandable abstracts use `<details class="pub-abstract">` with a `<div class="abstract-text">` inside.
...
- **Subpages** follow a consistent layout: nav → `<main>` → `<div class="page-header"><h1>` → content sections → `</main>` → footer. `index.html` differs by using a `.hero` section with a photo instead of `.page-header`.
